...
static void apply_double_steering(void)
{
    ...
    /* Multiple safety checks to prevent crash on race transitions */
    if (IsBadReadPtr(g_steerPtr, sizeof(float)) ||
        IsBadWritePtr(g_steerPtr, sizeof(float))) {
        g_steerPtr = NULL;
        g_calibState = 0;
        g_validFrames = 0;
        g_invalidCount = 0;
    ...
    /* If value isn't -1, 0, or 1, memory was reused — re-calibrate */
